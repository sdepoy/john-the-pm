...
interface DiscoveryPageProps {
  searchParams?: Promise<{ projectId?: string; threadId?: string }>
}
...
export default function DiscoveryPage({ searchParams }: DiscoveryPageProps) {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setThreadId(resolved?.threadId ?? null)
      setIsReady(true)
    })()
  }, [searchParams])
  ...
  const handleSend = () => {
    ...
    if (!text || status === 'streaming' || !projectId) return
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            Please create a project first or provide a projectId parameter.
          </p>
        </div>
      </div>
    )
